...
class c_n(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("그림_저거")
        self.setGeometry(200,200,640,100)
        ...
        self.p_label.setGeometry(210, 50, 400, 30)
    ...
    def dw_b(self,event,x,y,f,p):
        if event==cv2.EVENT_LBUTTONDOWN:
            cv2.circle(self.show_img,(x,y),5,self.L_C,-1)
            cv2.circle(self.mask,(x,y),5,cv2.GC_FGD,-1)
        elif event==cv2.EVENT_RBUTTONDOWN:
            cv2.circle(self.show_img,(x,y),5,self.R_C,-1)
    ...
    def f_s_b_f(self):
        # 파일 저장
        l_fname=QFileDialog.getSaveFileName(self,'파일 저장','./')
        cv2.imwrie(s_fname[0],self.cut_img)
    def e_b_f(self):
        # 종료
